from Cerina.modules.sql.captcha_sql import *
...
async def lel(event):
 ...
 if not args:
  ...
  x = set_mode(event.chat_id, mode)
  if not x:
    set_captcha(event.chat_id, "button")
...
async def lel(event, perm):
 ...
 if not args:
  ...
  mutetime = 0
  x = set_unmute_time(event.chat_id, mutetime)
...
async def lel(event, perm):
 ...
 if not perm.change_info:
         await event.reply("You are missing the following rights to use this command:CanChangeInfo!")
         return
 settings = get_style(event.chat_id)
 args = event.pattern_match.group(1)
 if not args:
  text = ""
  if settings == False:
...
"""
  ...
  await event.reply(text)
  x = set_style(event.chat_id, style)
  if not x:
   set_captcha(event.chat_id, style)
 else:
...
async def lel(event):
 ...
 if args:
  if args in avoid:
   return
 time = 300
 settings = get_time(event.chat_id)
 if not args:
...
async def lel(event, perm):
 ...
 if mutetime:
  ...
  x = set_time(event.chat_id, mutetime)
  if not x:
